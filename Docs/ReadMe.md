...
Via the context menu or by pressing F6 you can copy the name of the currently selected object in the hit list to the "Search for" control. This option is only available if a single row of the hit list is selected.
...
You must make sure that Fire is available in `⎕SE`. This can either be achieved by just executing the `]Fire` user command at least once or by executing `]Fire -noGUI`, which does load Fire into `⎕SE` but does not run Fire, meaning that the GUI would not show.

You can then execute the appropriate methods from the namespace `⎕SE.Fire`.
...
⍝ Version 20.0 or later
      (StartSearchIn:'⎕se') F.Search 'abc'
 ⎕SE.Parser        4 9.4
 ⎕SE.SALT          2 9.4
 ⎕SE.UnicodeFile   1 9.4
```

The result comes with three columns:
...
Since version 9.0 the API also allows you to search `⎕NL` recursively. Note that this searches `#` by default.
...
If you want to find "search" in the full path of the APL objects (like "Foo" in `#.Fist.Foo.MyFunction`), then you must set `FullPath` to 1.

In order to find "search" in `⎕SE` rather than `#` in version 20 and later:

```
      (StartSearchIn:'⎕SE') F.SearchQNL 'search'
```
...
Note that managing scripts with SALT is deprecated. Use Link instead.
